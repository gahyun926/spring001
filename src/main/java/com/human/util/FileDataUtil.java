...
public class FileDataUtil {
   ...
   private ArrayList<String> extNameArray = new ArrayList<String>()  //허용하는 확장자를 정의 한것
   {
      {
         add("gif");
         add("jpg");
   ...
   };      //<--현재코드는 활용하지안ㅎ는다. 얘는 선언이지 기능이 동작하지 않는다. 미리예측금지
   ...
   @ResponseBody //어떤 데이터를 포함하여 전송.. 어노테이션 .. view 지정하지 않고 바로 클라이언트 요청으로 응답
   ...
   public String[] fileUpload(MultipartFile[] file) throws IOException {
     String[] files = new String[file.length]; 
	   for(int i=0;i <file.length; i++) {
		   if(file[i].getOriginalFilename() != "") { //실제 파일 객체가 존제 한다면
			   String originalName = file[i].getOriginalFilename();//확장자가져오기위해 전체 파일명을 가져옴
